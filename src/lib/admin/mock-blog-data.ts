...
`,
  ...
  },
  {
    ...
    content: `# How to Permanently Redact a PDF: Free Secure Black-Box Censoring Guide
...
`,
  ...
  },
  {
    ...
    content: `# How to Calculate Home Loan EMIs in India: The Smart Homebuyer’s Financial Guide
...
export async function getDbPosts(): Promise<BlogPost[]> {
  ...
  if (supabase) {
    try {
      dbPosts = await retryQuery(async () => {
  ...
  // Deduplicate and combine, prioritizing Supabase DB posts over mock local array posts
  const combined = [...dbPosts];
  mockBlogPosts.forEach(mockPost => {
    if (!combined.some(p => p.slug === mockPost.slug)) {
...
export async function getDbPostBySlug(slug: string): Promise<BlogPost | undefined> {
  ...
  if (supabase) {
    try {
      const dbPost = await retryQuery(async () => {
        const { data, error } = await supabase
          .from("blog_posts")
          .select("*")
          .eq("slug", slug)
